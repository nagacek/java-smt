...
public class TheorySolving {
  ...
  private boolean solve(LogManager logger) throws InterruptedException, SolverException {
    ...
    UserPropagator theory = new InfoTheorySolver(logger, bfmgr, q);
    verify(prover.registerUserPropagator(theory));

    theory.registerExpression(p);
    theory.registerExpression(q);
    theory.registerExpression(p_v_q);
    theory.registerExpression(bfmgr.and(p, q));
